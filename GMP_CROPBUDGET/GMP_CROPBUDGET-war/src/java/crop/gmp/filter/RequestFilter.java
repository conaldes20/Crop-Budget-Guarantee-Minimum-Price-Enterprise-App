...
package crop.gmp.filter;

//import crop_budget.gmp.intface.CropBDGTRemoteBusiness;
//import crop_budget.gmp.services.UsersServices;
...
public class RequestFilter implements Filter {
   ...
   // private final static UsersServices userServices = new UsersServices();
	...
	HttpServletResponse res = (HttpServletResponse) response; 
        ...
        switch (spath) {
            case "/serviceslist.jsp":
                String service;
                service = "QtiesEntry";
                servicesA.add(service);
                service = "RatesEntry";
                servicesA.add(service);
                //service = "UpdateCropLocationLists";
                //servicesA.add(service);
                service = "ViewReport";
                servicesA.add(service);
                //service = "RatesUpdate";
                ...
                service = "AuditTrail";
                servicesA.add(service);
                service = "UserAccessControl";
                servicesA.add(service);
                CServices cservices = new CServices();
                cservices.setServies(servicesA);
                sess.setAttribute("cservices", cservices);
                request.setAttribute("servicesA", servicesA);
                System.out.println("request.setAttribute(\"servicesA\", servicesA): " + !servicesA.isEmpty());
            ...
            case "/reportList.jsp":
                ...
                if(locationsInfos == null)
                    locationsInfos = Collections.EMPTY_LIST;
                sess.setAttribute("locationsInfos", locationsInfos);
                request.setAttribute("locationsInfos", locationsInfos);
                //sess.setAttribute("locsInfos", locsInfos);
                break;
            case "/reportVars.jsp":
                //CRPLOCYRLists crplocyrLists = new CRPLOCYRLists();
                CRPLOCYRLists crplocyrLists = (CRPLOCYRLists)sess.getAttribute("crplocyrLists");
                List<String> crops = crplocyrLists.getCrops();
                List<String> locations = crplocyrLists.getLocations();
                List<String> years = crplocyrLists.getYears();
                if(crops == null)
	...
	// You might, for example, wish to add a setParameter() method. To do this
        ...
        public void setParameter(String name, String[] values) {
            if (debug) {
                System.out.println("RequestFilter::setParameter(" + name + "=" + Arrays.toString(values) + ")" + " localParams = " + localParams);
            }
        ...
        public String getParameter(String name) {
            if (debug) {
                System.out.println("RequestFilter::getParameter(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public String[] getParameterValues(String name) {
            if (debug) {
                System.out.println("RequestFilter::getParameterValues(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Enumeration getParameterNames() {
            if (debug) {
                System.out.println("RequestFilter::getParameterNames() localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Map getParameterMap() {
            if (debug) {
                System.out.println("RequestFilter::getParameterMap() localParams = " + localParams);
            }
            if (localParams == null) {
